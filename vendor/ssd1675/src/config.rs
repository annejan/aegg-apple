use crate::{
    command::{Command, DataEntryMode, IncrementAxis},
...
pub struct Builder<'a> {
    ...
    /// Optional: when `None`, the driver does not push a VCOM value
    /// during init.  Per-refresh `apply_lut_trailer` pushes VCOM from
    /// the active LUT's trailer byte so a Builder value is normally
    /// unnecessary; provide one only to pin VCOM at a fixed value
    /// across all refreshes.
    write_vcom: Option<Command>,
...
pub struct Config<'a> {
    ...
    pub(crate) write_vcom: Option<Command>,
...
impl<'a> Builder<'a> {
    ...
    /// Set VCOM register value (cmd 0x2C).
    ///
    /// Defaults to `None` — driver does not push a VCOM during init.
    /// On SSD1675B the per-refresh OTP-trailer dispatch (byte 104)
    /// handles VCOM and a Builder value is normally unnecessary.
    /// On SSD1675 (where VCOM is in a separate OTP region not read
    /// here) callers can pin VCOM via this setter; otherwise the
    /// chip keeps its POR / last-loaded value.
    pub fn vcom(self, value: u8) -> Self {
        Self {
            write_vcom: Some(Command::WriteVCOM(value)),
            ..self
        }
    }
    ...
    ///
    /// Defaults to DataEntryMode::IncrementYIncrementX, IncrementAxis::Horizontal. Corresponds to
    /// command 0x11.
